...
class SplashPotion extends Throwable {
    ...
    protected function onHit(ProjectileHitEvent $event) : void {
        ...
        if ($owningentity instanceof Player and $owningentity->isAlive()) {
            if ($owningentity->getWorld()->getBlockIdAt(round($owningentity->getX()), round($owningentity->getY())-1, round($owningentity->getZ())) === 0){
        ...
        $this->world->broadcastWorldEvent($this, WorldEventPacket::EVENT_PARTICLE_SPLASH, Color::mix(...$colors)->toARGB());
        $this->world->broadcastWorldSoundEvent($this, WorldSoundEventPacket::SOUND_GLASS);
        ...
        if ($hasEffects) {
            ...
            if ($blockIn->getId() === Block::FIRE) {
                $this->world->setBlock($blockIn, BlockFactory::get(Block::AIR));
            }
            foreach($blockIn->getHorizontalSides() as $horizontalSide) {
                if ($horizontalSide->getId() === Block::FIRE) {
                    $this->world->setBlock($horizontalSide, BlockFactory::get(Block::AIR));
                }
            }
        }
    }
